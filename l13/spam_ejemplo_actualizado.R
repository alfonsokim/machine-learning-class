...
setwd("/Users/Alfonso/r-workspace/machine-learning/l13")
...
joe <- as.matrix(read.csv("~/Documents/MCC/Aprendizaje/Proyecto/0.csv", header=F))
names(joe) <- c((paste("X", 1:256, sep="")), (paste("Y", 1:256, sep="")))
str(joe)
plot(joe)
head(melt(joe))
ggplot(melt(joe), aes(X1, X2, fill=value)) + 
...
ggplot(melt(testData), aes(X2,X1, fill=value))+xlab("MHz") + 
