...
# n=casos de entrenamiento, m=casos de prueba, beta=betas ajustadas
sim.1 <- function(n, m, beta=rnorm(100, 0, 0.1)){
    p <- length(beta)
    # rply evalua n veces la expresion. [,-1] es para cortar en contador 1..n
    # que devuelve rdply
    mat.entrena <- as.matrix(rdply(n, rnorm(p))[,-1]) + rnorm(p)
    mat.prueba <- as.matrix(rdply(m, rnorm(p))[,-1]) + rnorm(p)
    # Multiplicacion de la matriz de ent|pru por las betas, ajustar 
    # con la sigmoide. Despues distribucion binomial de las probabilidades
    g.entrena = rbinom(n, 1, h( mat.entrena %*% beta ))
    g.prueba = rbinom(m, 1, h( mat.prueba %*% beta ))
    list(mat.entrena=mat.entrena, mat.prueba=mat.prueba, 
         g.entrena=g.entrena, g.prueba=g.prueba)
}
